...
export default class GamePage {
        ...
        if (targetContainerIndex !== -1) {
          ...
          if (this.draggedElement.textContent) {
            const wordWidth = Math.max(
              50,
              this.draggedElement.textContent.length * 10
            );
  ...
  private handleWordCardClick(event: MouseEvent): void {
    ...
    if (
      clickedCard.classList.contains("word-element") ||
      clickedCard.classList.contains("new-container-element")
    ) {
      const sourceContainer = clickedCard.parentElement as HTMLElement;
      const targetContainer =
        sourceContainer === this.container
          ? this.newContainers[this.textExampleIndex]
          : this.container;
